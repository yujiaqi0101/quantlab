...
公式:
    SMA(下跌日 STD(CLOSE, 20), 20, 1)

公式解释:
    计算仅在下跌日的20日收盘价标准差的20日SMA平滑。
...
def alpha_160(
    ...
    公式: SMA(下跌日 STD(CLOSE, std_period), sma_n, sma_m)
    ...
    Returns:
        下跌日波动率因子面板 (date × symbol)
    """
    ret = ctx.get_returns()
    close_std = ts_std(ctx.close, std_period)
    # 仅在下跌日保留标准差，否则置为 NaN
    down_std = close_std.where(ret < 0)
    return sma(down_std, sma_n, sma_m)
